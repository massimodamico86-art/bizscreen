...
// YouTube video player widget for screen playback via iframe embed.
// Shows cached thumbnail + "Requires Internet" badge when offline.

import { useState, useEffect } from 'react';
import { extractYouTubeId, buildYouTubeEmbedUrl } from '../../../services/embedUtils.js';
import { getCachedMediaUrl } from '../../cacheService.js';
import { EmbedOfflineFallback } from './EmbedOfflineFallback.jsx';

/**
 * YouTubeWidget - Renders a YouTube video via iframe embed.
 *
...
export function YouTubeWidget({ props = {} }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    getCachedMediaUrl(`thumbnail:youtube:${thumbnailUrl}`)
      .then((blobUrl) => {
  ...
  if (!isOnline) {
    return <EmbedOfflineFallback thumbnailUrl={resolvedThumbnail} label="YouTube" />;
  }
