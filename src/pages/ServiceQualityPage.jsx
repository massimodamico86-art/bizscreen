...
export default function ServiceQualityPage() {
  ...
  const tierConfig = SLA_TIERS[subscriptionTier] || SLA_TIERS.free;
  ...
  const fetchSlaData = async () => {
    try {
      const [breakdown, status] = await Promise.all([
        getSlaBreakdown(tenantId, timeRange),
        getCurrentSlaStatus(tenantId),
      ]);
  ...
  const fetchAlerts = async () => {
    try {
      const data = await getCriticalAlerts(tenantId);
      setAlerts(data);
    } catch (error) {
  ...
  const fetchMetrics = async () => {
    try {
      const data = await getMetricsDashboard(tenantId, timeRange);
      setMetricsDashboard(data);
    } catch (error) {
  ...
  const fetchUsage = async () => {
    try {
      const data = await getUsageCounters(tenantId, 'daily');
      setUsageCounters(data);
    } catch (error) {
  ...
  const fetchNetworkHealth = async () => {
    try {
      const data = await getTenantNetworkSummary(tenantId, timeRange);
  ...
  const fetchErrors = async () => {
    try {
      const data = await getErrorBreakdown(tenantId, timeRange);
    ...
    <PageLayout>
      <PageHeader
        title={
          <Inline gap="sm" align="center">
            <Shield size={24} />
            {t('serviceQuality.title', 'Service Quality')}
          </Inline>
        }
        description={t('serviceQuality.description', 'SLA monitoring, alerts, and performance metrics')}
        actions={
          <Inline gap="md" align="center">
            ...
            <Button onClick={fetchAllData} disabled={refreshing} variant="ghost">
        ...
        <Stack gap="lg">
          ...
          <Grid cols={4} gap="md" className="grid-cols-1 md:grid-cols-2 lg:grid-cols-4">
            <SlaGaugeCard
              ...
              target={tierConfig.uptimeTarget}
              suffix="%"
              icon={<Activity size={20} />}
              trend={slaStatus?.uptimeTrend}
            />
            <SlaGaugeCard
              title="API Latency"
              value={slaStatus?.avgLatencyMs}
              target={tierConfig.latencyTarget}
              suffix="ms"
              icon={<Zap size={20} />}
              invertColors
              trend={slaStatus?.latencyTrend}
            />
            <SlaGaugeCard
              title="Device Online"
              value={slaStatus?.deviceOnlinePercent}
              target={tierConfig.deviceOnlineTarget}
              suffix="%"
              icon={<Monitor size={20} />}
              trend={slaStatus?.deviceTrend}
            />
            <SlaGaugeCard
              title="Webhook Success"
              value={slaStatus?.webhookSuccessPercent}
              target={tierConfig.webhookSuccessTarget}
              suffix="%"
              icon={<Bell size={20} />}
              trend={slaStatus?.webhookTrend}
            />
          </Grid>
          ...
          <Grid cols={3} gap="lg" className="grid-cols-1 lg:grid-cols-3">
            <div className="lg:col-span-2">
              <AlertsFeed
                ...
                alertingEnabled={tierConfig.alertingEnabled}
              />
            </div>
            <div>
              <NetworkHealthCard summary={networkSummary} />
            </div>
          </Grid>
