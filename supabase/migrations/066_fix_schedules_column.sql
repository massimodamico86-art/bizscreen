-- Migration 066: Fix schedules column name in template functions
--
-- Problem: apply_pack_template and apply_template use is_default column
-- but the schedules table has is_active instead.
--
-- Solution: Change is_default to is_active in both functions.
...
BEGIN
  ...
  -- Create schedules (FIXED: use is_active instead of is_default)
...
BEGIN
  ...
  LOOP
    ...
    -- Handle schedule blueprints (FIXED: use is_active instead of is_default)
...
COMMENT ON FUNCTION apply_template IS 'Instantiate a content template - fixed to use is_active column';
COMMENT ON FUNCTION apply_pack_template IS 'Instantiate a starter pack - fixed to use is_active column';

DO $$ BEGIN RAISE NOTICE 'Migration 066 completed: Fixed schedules column from is_default to is_active'; END $$;
