...
4. Verify realtime subscription is set up correctly:
   - Confirm useEffect in hook contains supabase.channel() subscription
   - Verify cleanup function calls supabase.removeChannel()
   - Check that handleScreenUpdate callback updates screens array on postgres_changes events
```bash
grep -A5 "supabase.channel" src/pages/hooks/useScreensData.js
grep "removeChannel" src/pages/hooks/useScreensData.js
```
...
Fix any issues before completing.
  ...
  <verify>
    ...
    - Realtime subscription with cleanup verified in hook
  </verify>
  <done>
    - All linting passes
