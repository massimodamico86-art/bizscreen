...
must_haves:
  truths:
    ...
    - "Correlation IDs propagate from components through services to API calls"
  artifacts:
  ...
  key_links:
    ...
    - from: "src/services/loggingService.js"
      to: "src/pages/*.jsx"
      via: "correlation ID propagation"
...
Purpose: Final verification that all success criteria are met. Escalate ESLint rule to error, remove deprecated logger.js, and add tests for logging infrastructure including correlation ID propagation.
...
<task type="auto">
  ...
  <action>
IMPORTANT: This task handles edge cases ONLY. If Plans 03-05 have incomplete tasks, those plans must be re-run first. This task assumes the bulk migration is complete.
...
2. Categorize findings:
   - **Missed files**: If more than 5 files have console calls, STOP and re-run the relevant plan (03, 04, or 05)
   - **Edge cases** (1-5 files): Migrate using established pattern
...
3. Fix any remaining edge case issues:
...
<task type="auto">
  <name>Task 3: Add logging infrastructure tests with correlation ID verification</name>
  <files>tests/unit/logging.test.js</files>
  <action>
Create tests/unit/logging.test.js to verify logging infrastructure, including correlation ID propagation:
...
describe('Correlation ID Propagation', () => {
  it('correlation ID is available for service layer', async () => {
    const { getCorrelationId, createScopedLogger } = await import('../../src/services/loggingService.js');
  ...
  it('different scoped loggers share the same correlation ID', async () => {
    const { getCorrelationId, createScopedLogger } = await import('../../src/services/loggingService.js');

    const logger1 = createScopedLogger('ComponentA');
    const logger2 = createScopedLogger('ServiceB');

    // Both loggers should produce logs with the same correlation ID
    const id1 = getCorrelationId();
    const id2 = getCorrelationId();
  ...
  it('correlation ID can be included in API calls via setLogContext', async () => {
    const { getCorrelationId, setLogContext } = await import('../../src/services/loggingService.js');

    // Simulate setting context for an API call
    const correlationId = getCorrelationId();
    setLogContext({ correlationId, userId: 'test-user' });
...
3. Correlation ID propagation tests pass
  </verify>
  <done>Logging infrastructure has test coverage verifying PII redaction, core functionality, and correlation ID propagation</done>
</task>
...
6. Success criteria from roadmap:
   ...
   - Correlation IDs in logs (verified by correlation ID propagation tests)
...
- All phase 4 success criteria verified:
  ...
  2. Logs include correlation IDs linking related operations (verified by tests)
