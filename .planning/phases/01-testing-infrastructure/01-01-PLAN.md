...
<task type="auto">
  ...
  <action>
Create the tests/mocks/ directory if it does not exist, then create a centralized Supabase mock module specifically for Player component tests.
...
Do NOT mock browser APIs here - those go in test setup.
  ...
  <verify>
Verify the mock module is syntactically valid:
- Run `node --check tests/mocks/supabase.js` to confirm no syntax errors
- Confirm the file exports the required functions by inspecting the export statements
  </verify>
  <done>
Supabase mock module exists at tests/mocks/supabase.js with RPC support for all Player.jsx RPC calls.
  </done>
</task>
...
- Mock `src/supabase` using the mock from tests/mocks/supabase.js (created in Task 1)
