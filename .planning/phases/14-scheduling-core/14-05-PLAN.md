...
<task type="auto">
  ...
  <verify>
    1. Run: `grep -n "@date-fns/tz" src/services/scheduleService.js` - should show import line
    2. Run: `grep -n "TZDate" src/services/scheduleService.js` - should show 2+ usages
    3. Run: `npm run build` - compiles without import errors
  </verify>
  <done>scheduleService.js uses TZDate for week preview date calculations, builds successfully</done>
</task>
...
<task type="auto">
  ...
  <verify>
    1. Run: `grep -n "@date-fns/tz" src/components/schedules/WeekPreview.jsx` - should show import line
    2. Run: `grep -n "TZDate" src/components/schedules/WeekPreview.jsx` - should show 5+ usages
    3. Run: `npm run build` - compiles without import errors
  </verify>
  <done>WeekPreview.jsx uses TZDate for all date navigation and calculations, builds successfully</done>
</task>
...
<task type="auto">
  ...
  <verify>
    1. Run: `grep -n "@date-fns/tz" src/components/schedules/DateDurationPicker.jsx` - should show import line
    2. Run: `grep -n "TZDate" src/components/schedules/DateDurationPicker.jsx` - should show 1+ usages
    3. Run: `npm run build` - compiles without import errors
  </verify>
  <done>DateDurationPicker.jsx uses TZDate for duration calculations across DST boundaries, builds successfully</done>
</task>
...
1. Run: `grep -l "@date-fns/tz" src/services/scheduleService.js src/components/schedules/WeekPreview.jsx src/components/schedules/DateDurationPicker.jsx` - should list all 3 files
2. Run: `grep -c "TZDate" src/services/scheduleService.js src/components/schedules/WeekPreview.jsx src/components/schedules/DateDurationPicker.jsx` - should show counts for each file
3. Run: `npm run build` - full build succeeds with no errors
4. Week preview shows 7 consecutive days (no gaps or duplicates)
5. Duration calculations produce correct end dates
