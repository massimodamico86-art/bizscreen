...
import { updateGroupLanguage, updateGroupLocation } from '../../services/screenGroupService';
```
...
- Add updateGroupLanguage(groupId, { display_language, location }) function:
```javascript
...
1. Add location column to screen_groups if not exists:
```sql
...
2. Update get_resolved_player_content to use group language as fallback:
...
3. Add comment explaining the inheritance chain:
```sql
...
This implements CONTEXT.md decision: "Strict inheritance — devices in group always use group's language, no override"
