...
<task type="auto">
  ...
  <action>
Update content fetch functions to include approval_status in select queries:

**For playlistService.js:**

1. Find fetchPlaylists (or fetchPlaylistsForPicker if it exists) and add approval_status to the select clause:

```javascript
// Look for select statements like:
// .select('id, name, description, ...')
// Update to include approval_status:
// .select('id, name, description, approval_status, ...')
```

Specific update - find the main fetch function and update:

```javascript
export async function fetchPlaylists(tenantId) {
  const { data, error } = await supabase
    .from('playlists')
    .select(`
      id,
...
If there are multiple fetch functions (fetchPlaylistsForPicker, fetchPlaylistsWithItems, etc.), update ALL of them to include approval_status.

**For sceneService.js:**

Apply the same pattern - find all fetch functions and add approval_status:

```javascript
export async function fetchScenes(tenantId) {
  const { data, error } = await supabase
    .from('scenes')
    .select(`
      id,
...
**Important:** Search for ALL .select() calls in both files and add approval_status to each one. The field was added by migration 122 (12-01-PLAN), so it will be available.
  </action>
  <verify>grep for "approval_status" in select queries in both playlistService.js and sceneService.js - should appear in ALL fetch functions</verify>
  <done>All content queries include approval_status field for UI display</done>
</task>
...
5. Content queries include approval_status field in playlistService.js
6. Content queries include approval_status field in sceneService.js
</verification>
