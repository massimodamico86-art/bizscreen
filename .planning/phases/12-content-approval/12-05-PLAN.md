...
**Navigation pattern:** The `onNavigate` prop should match DashboardPage's `setCurrentPage` signature: `(page: string) => void`. Based on DashboardPage.jsx line 84, the component receives `setCurrentPage` from App.jsx, which accepts a page name string like 'screens', 'playlists', 'review-inbox'.
  </action>
...
1. Add import near other component imports (around line 60-70):
```javascript
import { PendingApprovalsWidget } from '../components/dashboard/PendingApprovalsWidget';
```

2. Find the main content area in PageContent (around line 302) and add the widget after StatsGrid but before the two-column layout:

Based on the existing structure (line 350-358 shows StatsGrid, line 360 starts the two-column grid):
...
**Important:** Pass `setCurrentPage` as `onNavigate` - this is the navigation handler that DashboardPage receives from App.jsx (see prop definition on line 84).
  </action>
...
Read DashboardSections.jsx first to see its pattern.

If it re-exports external components, add:
...
If DashboardSections.jsx doesn't re-export external components (it may only define inline components), skip adding the re-export. DashboardPage already imports directly from the component file, which is fine.

The goal is to maintain consistent import patterns across the codebase. If DashboardPage imports directly from component files (which it does for WelcomeHero, WelcomeFeatureCards on line 69), that pattern is acceptable.
  </action>
