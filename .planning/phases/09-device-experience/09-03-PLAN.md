...
must_haves:
  truths:
    - "PIN entry displays full-screen numeric keypad (0-9 grid)"
    - "PIN is 4 digits with dot indicators"
    - "Wrong PIN shows 'Incorrect PIN' briefly with immediate retry"
    - "30-second inactivity auto-dismisses back to content"
  artifacts:
    - path: "src/player/components/PinEntry.jsx"
      provides: "Full-screen PIN entry component"
      exports: ["PinEntry"]
      min_lines: 100
  key_links:
    - from: "PinEntry.jsx"
      to: "validatePinOffline"
      via: "props.onValidate callback"
      pattern: "onValidate"
---

<objective>
Create PinEntry component with numeric keypad for kiosk exit

Purpose: Full-screen PIN entry interface for field technicians to exit kiosk mode
Output: ATM-style numeric keypad component with auto-timeout and error feedback
</objective>
...
@src/player/components/widgets/QRCodeWidget.jsx (styling pattern)
</context>

<tasks>

<task type="auto">
  <name>Task 1: Create PinEntry component</name>
  <files>src/player/components/PinEntry.jsx</files>
  <action>
Create new file with full-screen PIN entry UI:
...
Key features:
- Full-screen takeover with dark theme
- ATM-style keypad (1-9, 0, clear, backspace)
- 4-digit PIN with visual dot indicators
- Auto-validation when 4 digits entered
- Error feedback with immediate retry
- 30-second inactivity timeout
- Touch-friendly large buttons
- Cancel button for manual dismissal
  </action>
  <verify>File exists with PinEntry export: grep -n "export function PinEntry" src/player/components/PinEntry.jsx</verify>
  <done>PinEntry component created with numeric keypad, 4-digit PIN, error feedback, and 30s auto-timeout</done>
</task>

</tasks>

<verification>
1. Component file exists: `ls src/player/components/PinEntry.jsx`
2. Has proper exports: `grep "export function PinEntry" src/player/components/PinEntry.jsx`
3. Has 30s timeout: `grep "30000" src/player/components/PinEntry.jsx`
4. Has error state: `grep "Incorrect PIN" src/player/components/PinEntry.jsx`
5. Build passes: `npm run build`
</verification>

<success_criteria>
- [ ] PinEntry.jsx created with full-screen numeric keypad
- [ ] PIN_LENGTH = 4 with visual dot indicators
- [ ] Auto-validates when 4 digits entered
- [ ] Shows "Incorrect PIN" on wrong entry with immediate retry
- [ ] 30-second inactivity timeout calls onDismiss
- [ ] Cancel button calls onDismiss
- [ ] Touch-friendly button sizes (5rem)
- [ ] Build passes
</success_criteria>

<output>
After completion, create `.planning/phases/09-device-experience/09-03-SUMMARY.md`
</output>
